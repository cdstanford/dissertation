...
There are a vast number of DSPS, including many research prototypes as well as actively developed software in widespread use. Some of the major systems are listed in Table~\ref{fig:dsps-examples}.
Different systems often differ in some details, particularly with regard to implementation choices (e.g. scheduling and optimization strategies, fault tolerance mechanisms) and sometimes with regard to the abstractions that are offered to the programmer.

All DSPS in current use share the \emph{dataflow programming model.} This means that the programmer writes, in some form or another, a dataflow graph. In some systems, e.g. Apache Storm, the dataflow graph is written out explicitly, whereas in others, such as Apache Flink, the graph is implicit. Additionally, systems may offer high-level libraries for creating or composing dataflow graphs; in particular, these include libraries for complex windowing operations and for SQL- and CQL-based streaming queries.
...
\section{The Dataflow Programming Model}
...
\begin{defn}[Streaming Dataflow Graph]
...
\section{Auto-Parallelization (Operator Replication).}
...
\begin{defn}[Annotated Dataflow Graph and Parallelization]
...
\section{Streaming Runtime (Fault Tolerance and Scheduling)}
