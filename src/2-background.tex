...
Concretely, DSPS aim for latency in the milliseconds and throughput in tens of thousands of events per process per node.
...
DSPS use \emph{auto-parallelization} (also known as \emph{operator replication} or \emph{sharding}) to accomplish data parallelism, and unlike the other two kinds of parallelism, it modifies the dataflow graph and potentially the semantics of the program.
...
Given fixed resources, there is always some limit to how much input the system can handle; so max throughput is always finite. Beyond the max throughput, DSPS offer few guarantees about behavior, and will likely suffer increasing latency, drop data, or crash altogether.
...
However, most DSPS do not enforce annotations to determine when auto-parallelization is safe or not~\cite{schneider2013safe}.
...
Related to state management, DSPS also promise a programming model
