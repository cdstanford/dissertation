...
then an SPST is a (type-safe and deterministic) function from input batches to output batches, which respects the structure given by the types. This material was originally presented in~\citeMain{pods21} (some of which was delegated to the appendix).
...
$P_2$ keeps the last known location for the taxi and the current distance travelled as its state, and each time it processes a new \code{GPS} tuple, it updates both.
...
\spstfield{P_2}{update}((loc, d), gps)
    &= (( loc, d), \code{Outlier}(loc)) \text{ otherwise}
\end{align*}
...
and then simply divides them to return the \code{float} ride cost per travelled mile, i.e., $\spstfield{P}{fin}(dist, cost) = cost / dist$.
...
The following is the major technical result of this chapter: for any SPST, the open semantics is monotone in the prefix relation on partially ordred streams.
