...
semantics~\cite{schneider2013safe}~\citeMain{pldi19}.
...
\section{Geo-distributed streaming RW}
...
Other than~\citeMain{cardellini2016optimal}, there are several lines of work in job scheduling, operator placement, and optimization for DSPS that try to be network-aware in some fashion. Early and influential works include~\cite{ahmad2004network} and~\cite{pietzuch2006network}.
...
The work~\cite{gu2015general}, similarly to~\citeMain{cardellini2016optimal}, formulates the problem in MILP.
...
%% Decided to not cite these as surveys, since I don't think they are good surveys. Just incorporated into the above text.
% (see also the surveys~\cite{dias2018dsp-survey,lakshmanan2008placement}).

\paragraph{Programming Frameworks.}
A system very related to SpanEdge~\citeMain{sajjad2016spanedge}
...
The first stream processing system to incorporate the degradation of data streams as found in~AWStream~\citeMain{zhang2018awstream} was JetStream~\cite{rabkin2014jetstream}.
