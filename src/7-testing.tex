...
This problem is not easy. One issue is that the property we are interested in can be thought of as ``on all executions, the ordering of the stream is the same up to equivalence'' which is a property that requires running the program mutiple times (a hyperproperty).
...
\begin{figure}[t]
  \centering
  \begin{tikzpicture}
    \matrix (m) [matrix of math nodes, column sep=2em, row sep=0em]
    {
              &         & |(a3)|a &         \\
      |(a1)|a &         &         & |(b2)|b \\
              & |(b1)|b & |(a4)|a &         \\
...
% \cref{diffstream:fig:keybytest} shows an example test written using DiffStream for the taxi example.
% \cref{diffstream:fig:mapreduce-case-study-summary} shows the results of a case study on MapReduce programs to find bugs due to nondeterminism.
% Finally, \cref{diffstream:fig:online-perf-results} shows the results of the final case study on measuring the performance overhead of the algorithm.
%
% \begin{figure}[tb]
%     \centering \small
% \begin{tabular}{c}
...
This also raises two interesting possibilities for extension that I currently think would be promising:
\begin{itemize}
