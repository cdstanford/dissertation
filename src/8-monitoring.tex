...
\caption{Data transducer $\DT_2$ monitoring the stream to produce, at the end of each day, either the average price of an $\tg{a}$-item (if there was at least one $\tg{a}$) or the previous average (if there was no $\tg{a}$). When there are multiple transitions $q' := t_1$ and $q' := t_2$, the semantics is such that we assign $q' := t_1 \sqcup t_2$.}
...
\caption{Data transducer $\DT_3$ monitoring the stream to produce, at the end of each day, the difference between the maximum price of an $\tg{a}$-item and the maximum price of a $\tg{b}$-item.}
