...
The success of stream processing APIs based on the data\-flow model can be attributed to their ability to simplify the task of parallel programming. To accomplish this, most APIs expose a simple but effective model of data-parallelism called \emph{sharding}:
...
In order to evaluate DGS, we perform a set of experiments to investigate the data parallelism limitations of Flink~\cite{carbone2015flink}---a representative high-performance stream processing system---and
...
\subsection{Dependency-Guided Synchronization}
...
First, equation (C1)
is over all
joins \fl{join: (State_j, State_k) ->} \fl{State_i},
...
\subsubsection{Applications requiring synchronization}
...
\subsubsection{Implementations in Flink and Timely}
...
\subsubsection{Manual synchronization}
...
%         // ...
  \begin{lstlisting}[language=Java,basicstyle=\scriptsize\ttfamily,commentstyle=\scriptsize\ttfamily,morekeywords={var}]
...
\subsubsection{Implementation in Flumina}
...
\begin{table*}[t]
  % \normalsize % Caleb prefers
  \small % Konstantinos prefers
  \begin{center}
    % Compromise
...
\subsubsection{Summary: development tradeoffs}
...
In this section, we evaluate if DGS can be used for realistic application workloads.
...
\subsubsection{IoT power prediction}
...
\Cref{sec:prog-model}, where we maintain a map of historical totals
...
\begin{figure*}[t]
  \begin{tikzpicture}[sibling distance=25em, level distance=80pt,
    every node/.style = {shape=rectangle,
      rounded corners,
      draw, align=center},
    level 1/.style = {sibling distance=24em},
  ...
  \end{tikzpicture}
  \caption{Example synchronization plan generated in \Cref{example:optimization}.
...
We prove \Cref{theorem:correctness} and therefore show that our framework is correct according to \Cref{def:distr-correctness}.
The key assumptions used are:
\begin{enumerate}
\item[(1)] The program is consistent, as defined in \Cref{ssec:prog-model-correctness}.
...
\begin{figure}[t]
    ...
    \label{fig:synchronization-overhead}
\end{figure}
...
\subsubsection{State checkpoints}
...
snapshots~\cite{murray2013naiad,carbone2017statemanagementflink},
