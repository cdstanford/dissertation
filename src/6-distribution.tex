...
% and understandable than threads based on a simple principle: \emph{Deterministic ends should be accomplished with
% deterministic means.}}{Edward A. Lee, 2006~\cite{lee2006problem}}
  \headerquote{Deterministic ends should be accomplished with deterministic means.}{Edward A. Lee, 2006~\cite{lee2006problem}}
}

This section considers the problem of automatic distribution of streaming operators.
Rather than the structured representation of \Cref{cha:composition},
...
The success of stream processing APIs based on the dataflow model can be attributed to their ability to simplify the task of parallel programming. To accomplish this (as described in \Cref{background:auto-parallelization}), most APIs expose a simple but effective model of data-parallelism called \emph{sharding} (\emph{auto-parallelization}), in which
...
that is, any concrete implementation that corresponds to a $P$-valid plan is equivalent to a program $P$ that satisfies the consistency conditions.
...
In \Cref{dgs:appendix:case-studies},
...
Some of the material in this section (\Cref{dgs:appendix:case-studies,dgs:outtakes}) was published only in the extended version of the original paper~\citeMain{flumina-arxiv}, as an appendix.
...
In \Cref{dgs:ssec:prog-model-correctness} we define program \emph{consistency}, which consists of requirements on the \emph{fork} and \emph{join} functions to ensure that any parallel implementation generated from the program is equivalent to the sequential one.
...
The
fork function additionally takes as input two predicates on events,
...
($\tg{f}$ and $\tg{j}$ denote forks and joins).
}
...
Let \fl{l1 + l2} be list concatenation and let \fl{inter(l, l1, l2)}
be list interleaving, as defined in \Cref{cha:foundation}.
...
\[
\fl{out(s1, e)} = \fl{out(join(s1, s2), e)}. \tag{C4}
\]
Equation (C2)
...
framework for generating them a the given DGS program
...
progress by processing these dependent events in order.
...
\fl{State}), and a triple of update, fork, join functions.
...
Its design is described in more detail in \Cref{dgs:appendix:optimizer}.
...
(1) \emph{Monotonicity:} for all $i$, \fl{u_i} is in strictly increasing order according to $\mathrel{<_{\mathcal{O}}}$.
  (2) \emph{Progress:} for all $i$, for each non-heartbeat input event \fl{x} in \fl{u_i},
...
To prove \Cref{dgs:theorem:correctness}, the key assumptions used are:
\begin{enumerate}
...
\Cref{dgs:thm:consistency-implies-determinism} as well as
...
public Integer joinChild(
  ...
  \end{lstlisting}
  \end{minipage}
  \caption[Flink code snippet.]{Snippet from the implementation of the manual synchronization \fl{join} in Flink. This implementation does not satisfy \textbf{PIP1--3}.}
...
We implemented all three applications
in Flumina, and measured the maximum throughput increase with the addition of parallel nodes
...
parallel implementation is a straightforward pair of \fl{fork} and fl{join} primitives, which amounts to 50~LoC.
...
parallelization is modest compared to the sequential code.
...
\subsection{Flumina Synchronization Latency}
...
From the perspective of the structured types and type safety in \Cref{cha:foundation},
...
The core programming model of \Cref{dgs:sec:prog-model} is elegant, and one thing we have wondered is whether it can be used to get consistency guarantees in distributed programming. Related and recent work in consistent replicated distributed systems
(e.g., RedBlue consistency~\cite{li2012making},
MixT~\cite{milano2018mixt}, Gallifrey~\cite{milano2019tour}, Hamband~\cite{houshmand2022hamband}, and Quark~\cite{kaki2022runtime})
