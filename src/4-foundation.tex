...
\begin{enumerate}
\item[(i)] $\event{S_1}$ is a subtype of $\event{S_2}$
(i.e., for all $\eventtype{e}{S_1}$, $\eventtype{e}{S_2}$); and
\item[(ii)] for all tuples $\eventtype{x, y}{S_1}$,
...
This brings us to the theorem. The subtyping problem for stream types is to determine, on input two grounded stream types $S_1$ and $S_2$, whether $S_1 \subtype S_2$. Formally:
\[
\textsc{Subtyping} = \Big\{ \langle S_1, S_2 \rangle \;:\; S_1, S_2 \text{ are grounded stream types and } S_1 \subtype S_2. \Big\}
\]

\begin{theorem}
\label{thm:stream-relaxation-decidable}
\textsc{Subtyping} is decidable in quadratic time.
\end{theorem}
\begin{proof}
...
Consequently, checking order-equivalence is decidable in quadratic time as it is sufficient to check relaxation in both directions.
