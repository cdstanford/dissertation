...
To illustrate monotonicity, consider the function \textsc{Sum} (encoded as an operator as a set of ordered pairs), which takes in a sequence of integers and outputs their sum. \textsc{Sum} is not monotonic because on input $[1, 2, 3]$ it produces $[6]$ and on input $[1, 2, 3, 1]$ it produces $[7]$, but $[6]$ is not a prefix of $[7]$. To make it monotonic, we have to add an end-of-stream symbol at the end: the correct function to consider is the one which maps $[1, 2, 3]$ to $[]$, but maps $[1, 2, 3, \#]$ to $6$.
...
To illustrate type safety, consider the function \textsc{Id}, which consists of all pairs $(l, l)$, e.g. it maps $[1, 2, 3]$ to $[1, 2, 3]$.
...
Finally, to illustrate determinism, it is immediate that \textsc{Sum} and \textsc{Id} are deterministic because they are functions. But for a less trivial example, consider the relation \textsc{Forget}, which maps a list $l$ to all its possible reorderings: in particular, for $l = [1, 2, 3]$, \textsc{Forget} contains all six pairs
\[
...
\begin{tabular}{ccccccc}
  ...
  \textsc{Sum}
    & $[1, 2, 3] \mapsto [6]$
    & \seqleaf{\texttt{int}} & \seqleaf{\texttt{int}}
    & No & Yes & Yes \\
  \textsc{SumTerminated}
    & \makecell{$[1, 2, 3] \mapsto []$ \\ $[1, 3, 2, \#] \mapsto []$}
    & $\seqleaf{\texttt{int + \#}}$ & $\seqleaf{\texttt{int}}$
    & Yes & Yes & Yes \\
  \textsc{Id}
    & $[1, 2, 3] \mapsto [1, 2, 3]$
...
\caption[Examples of monotonicity, type-safety, and determinism.]{Examples of monotonicity, type-safety, and determinism for a selection of stream operators and input and output types.}
\label{fig:operator-properties-examples}
\end{figure}

It is possible for an operator to be type safe, but not deterministic. For example, if $\equivtype{[1, 2, 3]}{[1, 3, 2]}{S}$ but NOT $\equivtype{[1, 2, 3]}{[1, 3, 2]}{S'}$, then \textsc{Forget} is not deterministic, but \emph{is} still type safe because the output, while a set of inequivalent possibilities and not just one, doesn't depend on the input.

It is also possible for an operator to be deterministic, but not type safe.
For example, consider the last-element function \textsc{Last} that on $[1, 2, 3]$ returns $[3]$ and on $[1, 3, 2]$ returns $[2]$. This is deterministic because it is a function. But if we have $\equivtype{[1, 2, 3]}{[1, 3, 2]}{S}$, then it is not type safe, because $[2]$ and $[3]$ are not equivalent outputs. (It's also not monotonic, but we could change the example ot be monotonic by delaying output until receiving an end-of-stream marker $\#$, and it will still not be type safe.)
...
In addition to the characterization three properties above obey some compositionality laws.

% TODO: discuss compositionality of properties
% monotonicity is compositional
% type-safety is compositional
% Determinism is only compositional assuming type safety!
% full determinism would be compositional.
...
\ref{cha:testing}
  ...
  & --- & \makecell{Partial \\ (For a specific \\ runtime trace) \\ Thm.~\ref{diffstream:thm:correctness}} & \makecell{Partial \\ (For a specific \\ runtime trace) \\ Thm.~\ref{diffstream:thm:correctness}} \\
\ref{cha:monitoring}
  & \makecell{Restricted \\ (DT or QRE-Past)} & $\seqleaf{T}$
  & \makecell{Yes \\ Thm.~\ref{dt:thm:dt-eval}} & --- & Yes \\
\end{tabular}

\caption[Monotonicity, type safety, and determinism for the remaining chapters.]{Monotonicity, type safety, and determinism for operators defined in the remaining chapters. Here, $F$ is an operator from $S$ to $S'$ defined in a domain-specific language (DSL) specific to each chapter (or any black-box program for \Cref{cha:testing}). Entries marked --- indicate that the property is not considered or does not apply for the DSL in question.}
