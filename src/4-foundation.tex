...
\ref{cha:testing}
  ...
  & --- & \makecell{Partial \\ (For a specific trace)} & \makecell{Partial \\ (For a specific trace)} \\
\ref{cha:monitoring}
  & \makecell{Restricted \\ (DT or QRE-Past)} & $\seqleaf{T}$
  & Yes & --- & Yes \\
\end{tabular}

\caption[Monotonicity, type safety, and determinism for the remaining chapters.]{Monotonicity, type safety, and determinism for operators defined in the remaining chapters. Here, $F$ is an operator from $S$ to $S'$. $---$ means the property is not considered or does not apply for the formalism in that chapter.}
