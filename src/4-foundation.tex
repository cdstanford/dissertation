...
They are based on \emph{synchronization schemas}~\citeMain{pods21}, which in turn evolved from our earlier work on \emph{data-trace types}~\citeMain{festschrift18,pldi19}.
...
Our main results include \Cref{prop:batch-lin-correspondence,prop:lin-poset-correspondence}, which state that batches, linearizations and posets are all isomorphic for any type $S$.
\Cref{prop:stream-types-less-general} characterizes the equivalence relations (on linearizations) that can arise from a stream type.
Finally, \Cref{thm:stream-relaxation-decidable} states that subtyping for stream types is decidable.

To connect these abstract results to real systems, we also consider a definition of \emph{operators} (similar to the definition in \Cref{cha:background}), which are nondeterministic functions from linearizations to linearizations.
We define three key properties for operators: monotonicity, type safety, and determinism. We connect these properties to the rest of the thesis in \Cref{fig:operator-properties-chapter-table}.
\Cref{prop:compositionality-laws,prop:compositionality-laws-additional} state compositionality laws for these properties (under sequential and parallel composition).
Lastly, \Cref{thm:type-safe-and-deterministic-consequence}
captures why the properties matter: type-safe, deterministic operators give rise to well-defined stream transformations -- well-typed functions on batches and labeled posets.

We choose to model streams as finite objects -- in contrast to the traditional view as coinductive or infinite sequences. Though our results should adapt to the infinite setting, there are two reasons for this choice. First, infinite streams are not necessarily useful in practice, since real streams do eventually end -- if nothing else, a DSPS will typically provide a way to take down the whole system and flush all output, e.g., via an end-of-stream punctuation event.
Second, even in cases where we have in mind a truly infinite stream, relevant substreams still need to be finite. For example, in a stream of values separated by barriers, each block of values should be finite to ensure progress.
The trade-off for using finite streams is that we need to recover monotonicity as a property (\Cref{sec:operator-properties}), whereas for infinite streams monotonicity would come ``for free,'' since operators over infinite streams can only work in an item-by-item fashion.
...
GPS data for each taxi is a tuple type $\bigcirc =$ \code{GPS(x: float, y: float, z: float)}, which indicates the type of a GPS reading using 3-dimensional coordinates.
Completed ride data is a tuple type $\triangle =$ \code{RideCompleted(rideID: int, passengerID: int, cost: int)}.
Finally, end-of-hour events are used to synchronize in time; these are a tuple type $\blacksquare =$ \code{EndOfHour(date: date, hour: int)}.
...
Described from the bottom up: first, the type $S_1 = \keyby{\code{taxiID}}{\seqleaf{\code{GPS}}}$ denotes that \code{GPS} events are partitioned by the key \code{TaxiID} and are totally ordered for each taxi.
Second, $S_2 = \relleaf{\code{RideCompleted}}$ denotes that \code{RideCompleted} events are unordered, and can be considered to be a bag.
Finally, $S = \hier{\code{EndOfHour}}{\parcomp{S_1}{S_2}}$ denotes that \code{EndOfHour} events synchronize the events in $S_1$ and $S_2$, each of which can be processed in parallel as they are independent.
\end{example}
...
\begin{mathpar}
    ...
    {
      \batchtype{b_0}{S} \\
      t_i: T \text{ and } \batchtype{b_i}{S} \text{ for all } i = 1, \ldots, m
    }
    {
    ...
    \inference[ParBy]
    {
      k_i: K \text{ for all } i \\
      k_i \ne k_j \text{ for all } i \ne j \\
      \batchtype{b_i}{S} \emph{ nonempty} \text{ for all } i
    }
    {
...
The \emph{nonempty} requirement in the \textsc{ParBy} means that $b_i$ must have at least one occurrence of $t: T$ for some base type $T$.
...
In the \textsc{Synch} case, a batch is a list of odd length: it is formed from $m$ synchronizing elements of type $T$ and $m + 1$ batches of type $S$, for some $m \ge 0$.
...
On the other hand, it \emph{would} be useful in practice to have a version of $\hier{T}{S}$ that requires the stream to be terminated by an element of $T$, as this avoids the off-by-one mismatch between elements of $T$ and batches of $S$.
But this requires a generalization of the type system, to add other constructs including singleton types; see \Cref{sec:types-discussion}.
...
We say that a stream type $S$ is \emph{grounded} if all base types $T$ occurring in $S$ are nonempty and disjoint.
...
For completeness, such a typing judgment is shown in \Cref{omitted:typing} for grounded stream types.
...
The other important thing to check immediately is that concatenation respects equivalence, but we don't need to prove this as it is baked into the \textsc{Concat} rule. The rule forces that equivalent linearizations concatenate to get equivalent linearizations.

\subsection{Streams as Labeled Posets}
\label{view:labeled-posets}

For our last view of a stream, we define the concept of a \emph{labeled poset}. This concept is traditionally called a \emph{pomset}, for partially ordered multiset; however, this terminology can be confusing, because the ordering relation is on the underlying set, not on the elements of the multiset.\footnote{
...
\item[(ii)] No strictly smaller ordering (strict subset of $\le$ that is still a partial order) satisfies (i).
\end{enumerate}

For example, in \Cref{fig:ex-postream}, (i) states that there must be a path between dependent events: there must be a path between every pair of red circles, or between every triangle and black square.
...
We illustrate the different views of streams with the example of values and barriers from \Cref{ex:value-barrier}. The stream type corresponding to this example is
\[
S = \hier{\#}{\relleaf{\code{int}}}
\]
where $\#$ denotes a barrier and \code{int} denotes a value (integer).
...
The types are isomorphic in the sense that there is a one-to-one correspondence between their elements which also preserves the structure; in particular, we show that it preserves a concatenation operation.
To define an isomorphism between batches and linearizations,
we need a \emph{flattening} relation which gives, for each batch, at least one (and possibly more than one) linearization corresponding to that batch.
...
\end{proposition}
\begin{proof}
By the \textsc{Concat} rule as remarked earlier, if $\equivtype{l_1}{l_1'}{S}$ and $\equivtype{l_2}{l_2'}{S}$
...
This definition captures the property that $l[i]$ and $l[j]$ are ordered through some transitive closure of dependencies between events in the linearization. And when this holds, it is impossible to reorder the events ($\equiv$) to get $l[j]$ before $l[i]$.
...
The reverse direction requires decomposing any isomorphism by induction into a matching between the elements of the individual linearizations such that the matching can be formed by repeated consecutive swaps, implying equivalence via \textsc{Indep}, \textsc{Concat}, and \textsc{Trans}; this is the same as for data-trace types and a full proof can be found in~\citeMain{pldi19} and~\citeMain{festschrift18}.
...
\begin{enumerate}
\item[(i)] For every base type $T$ occurring in $S_1$, $T$ occurs in $S_2$;
...
We first state the groundedness result, then we get to the main theorem, \Cref{thm:stream-relaxation-decidable}.
Define
\[
\textsc{Groundedness}
...
As a corollary of \Cref{thm:stream-relaxation-decidable}, checking order-equivalence is decidable in quadratic time as it is sufficient to check subtyping in both directions.
...
All of these examples, and a few extras, are summarized in \Cref{fig:operator-properties-examples}.
...
We include a few more examples, omitted from this section, in \Cref{sec:outtakes-examples}.
...
If $F$ is both type-safe and deterministic, then the following theorem says it is a \emph{well-defined function} from input streams up to equivalence to output streams up to equivalence.
...
The converse of \Cref{thm:type-safe-and-deterministic-consequence} is also true: any well-defined function on batches or labeled posets is, when interpreted as a function on linearizations, both type-safe and deterministic. We state the result for batches; the result for posets is analogous.
\begin{theorem}
...
Additional compositionality laws can be given for other kinds of composition, in particular parallel composition; see \Cref{sec:additional-compositionality-laws}.
...
Among the different DSLs, \Cref{cha:distribution} is relatively expressive as a language (in particular, it allows arbitrary sequential programs), whereas the others are relatively restricted.
...
Writing further operations on the result of an aggregation, such as dividing a sum and a total count to get an average (\code{let avg = sum / count}), is awkward when the arguments are streams and not known to be singletons.
...
\begin{proposition}
\label{prop:compositionality-laws-additional}
...
\caption[Outtakes from \Cref{fig:operator-properties-examples}.]{Outtakes from \Cref{fig:operator-properties-examples}: additional examples of monotonicity, type safety, and determinism for stream operators.}
\label{fig:outtakes-examples}
\end{figure}

Figure~\ref{fig:outtakes-examples} shows some additional outtakes to add to the examples listed in \Cref{fig:operator-properties-examples}.
...
or, that $a$ is an ancestor of $b$ in the tree visualization of the stream type (\Cref{45:fig:example-schema}).
...
To formalize this relation, only a single change to the rules in \Cref{view:events} is required, in the \textsc{Synch} case, to make it asymmetric.
The modified rule is as follows:
\begin{mathpar}
    \inference[Synch-asymmetric]
    {
      t: T \\
      \eventtype{e}{\hier{T}{S}}
    }
...
Below we include the typing rules for $\indeptype{e}{e'}{S}$, omitted from \Cref{view:events}.
...
The stream types defined in this chapter differ from what we called \emph{synchronization schemas}~\citeMain{pods21}
in a few ways. Mostly these are not conceptually fundamental differences, but they nevertheless have some consequences. We survey each difference individually and also discuss the relationship with \emph{data-trace types}~\citeMain{pldi19}.
...
Here is the definition of \emph{header} from~\citeMain{pods21},
...
In our development, at least for the batch view, our stream types don't need to satisfy any well-formedness conditions; for equivalence with linearizations though (\Cref{prop:batch-lin-correspondence}), we do need a condition analogous to (1) (groundedness, i.e. that base types in a schema are disjoint). We don't need conditions (2) and (3).

\subsection{Batches vs. Series-Parallel Streams}

The batches we define ($\batchtype{b}{S}$) are analogous to what were called
\emph{series-parallel streams} (or SPSs) in~\citeMain{pods21}, the type inhabitants for synchronization schemas.
...
\paragraph{Series-parallel streams:} Consider the sequence of \code{GPS} events corresponding
to the red taxi. The type of this sequence is $\spstype{\code{taxiID}=\code{red}}{\seqleaf{\code{GPS}}}$. Such a type can
be viewed as a \emph{refinement type} of the schema type \seqleaf{\code{GPS}}.
This type is more specialized than \seqleaf{\code{GPS}},
since all these events share a common value of the field \code{taxiID}.
...
Additionally, \citeMain{pods21} defined \emph{concatenation} of series-parallel streams, denoted $\circ$.
In our setting we instead derive $\circ$ from concatenation on linearizations rather than defining it inductively.

\begin{definition}[Concatenation and Prefix Ordering for SPSs]
...
This is not substantially different, but does differ in one important way: it is guaranteed in~\citeMain{pods21} to be reflexive, that is $\deptype{e}{e}{S}$ for all events $\eventtype{e}{S}$.
As remarked in~\citeMain{pods21} and included below (see \Cref{prop:why-reflexive}), this has some advantages.
...
which is analogous to our development in \Cref{prop:batch-lin-correspondence}.
...
Here is the definition of a data type, dependence relation, and equivalence relation from \citeMain{pldi19}, where the latter bears close resemblance to equivalence on linearizations.
