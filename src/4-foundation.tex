...
Lastly, to connect these abstract results to real systems, we also consider a definition of \emph{operators} (similar to the definition in \Cref{cha:background}), which are nondeterministic functions from linearizations to linearizations.
We define three key properties for operators, monotonicity, type safety, and determinism, and connect these properties to the rest of the thesis in \Cref{fig:operator-properties-chapter-table}.
...
Finally, \Cref{thm:type-safe-and-deterministic-consequence}
...
\emph{series-parallel streams} (or SPS) in~\citeMain{pods21}, the type inhabitants for synchronization schemas.
