...
The \emph{nonempty} (not empty) requirement in the \textsc{ParBy} case is the inductive property that fails exactly when the batch has no occurrences of $t: T$ for any base type $T$.
Specifically, $(b_1, b_2)$ is empty if $b_1$ and $b_2$ are empty;
a list is empty iff each of its elements is empty;
and a bag is empty iff each of its elements is empty.
...
Let $S$ be a stream type such that all base types in $S$ are disjoint.
\begin{enumerate}
...
Stream type relaxation (on input two stream types $S_1$ and $S_2$, checking if $S_1 \lesssim S_2$) is decidable in quadratic time.
\end{theorem}
\begin{proof}
...
Let $F_1$ be an operator from $S_1$ to $S_1'$, and let $F_2$ be an operator from $S_2$ to $S_2'$. Then:
\begin{enumerate}
