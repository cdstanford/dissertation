...
GPS data for each taxi is a tuple type $\bigcirc =$ \code{GPS(x: float, y: float, z: float)}, which indicates the type of a GPS measurement using three dimensional coordinates.
...
The \emph{nonempty} requirement in the \textsc{ParBy} case is the inductive property that holds exactly when the batch has no occurrences of $t: T$ for any base type $T$.
...
i.e. $\event{S} \times \event{S} \to \code{bool}$.
...
Then $l$ is a flattening of $b$ if and only if $l$ is an interleaving of some $l_1, l_2$ where $l_1$ is a flattening of $b_1$ and $l_2$ is a flattening of $b_2$.
...
Stream type relaxation, that is on input two stream types $S_1$ and $S_2$, checking if $S_1 \lesssim S_2$ (and consequently checking stream type equivalence), is decidable in quadratic time.
\end{theorem}
\begin{proof}
...
to the red taxi. The type of this sequence is \seqleaf{\code{GPS}}, but is more specialized
...
as $\spstype{\code{taxiID}=\code{red}}{\seqleaf{\code{GPS}}}$. Such a type can
