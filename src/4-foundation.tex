...
\begin{theorem}
Let $F$ be both type safe and deterministic.
Then there exists a unique functions (1) $f: \batch{S} \to \batch{S'}$ and (2) $g: \poset{S} \to \poset{S'}$
TODO
\end{theorem}
\begin{proof}
Application of \Cref{prop:batch-lin-correspondence} and \Cref{prop:lin-poset-correspondence}.
\end{proof}

The converse is also true: any well-defined function on batches or labeled posets is, when interpreted as a function on linearizations, both type safe and deterministic.
\begin{theorem}
Let $f: \batch{S} \to \batch{S'}$ be any function, and define $F$
to be the set of pairs $(l, m)$ such that $f(\parselin{l})$ TODO
\end{theorem}
\begin{proof}
TODO
\end{proof}

The consequence of monotonicity is that the operator not only implements a well-defined function, but also is \emph{streamable} in the sense that the output can be produced incrementally. TODO
