...
Say that an \emph{operator} from type $A$ to $B$ is a possibly nondeterministic program
$f$ from \texttt{List(Event)} to \texttt{List(Event)} that is \emph{monotone}:
If $l$ is a prefix of $l'$, then every output in $f(l)$ is a prefix of some output in $f(l')$.

Given an input stream type $S$ and output stream type $S'$:
\begin{itemize}
  \item \textbf{Type safety:} $f$ is \emph{linearization type safe} if whenever $\lintype{l}{S}$, for every $m$ in $f(l)$, $\lintype{m}{S'}$.
  \item \textbf{Determinism:} $f$ is \emph{deterministic up to equivalence} if whenever $\equivtype{l}{l'}{S}$, for every $m$ in $f(l)$ and $m'$ in $f(l')$, $\equivtype{m}{m'}{S}$.
\end{itemize}
