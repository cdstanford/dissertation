...
\section{Stream Relaxation}

Stream relaxation is a form of semantic subtyping: $S_1$ is a relaxation of $S_2$ means that a stream of type $S_2$ can be interpreted as a stream of type $S_1$ if the partial order is relaxed.
...
For synchronization schemas $S_1$ and $S_2$,
...
Two synchronization schemas $S_1$ and $S_2$ are \emph{order-equivalent}, denoted $S_1 \sim S_2$, if both $S_1 \lesssim S_2$ and $S_2 \lesssim S_1$.
\end{definition}
...
Stream relaxation, that is on two input schemas $S_1$ and $S_2$, checking if $S_1 \lesssim S_2$, and consequently checking schema equivalence, is decidable in quadratic time.
\end{theorem}
\begin{proof}
...
\section{Type Safety and Determinism}

Finally, the true purpose of this chapter is to formally define type safety and determinism. Stateful operators on streams are often given in an element-by-element fashion, though this is not compositional. We will describe operators this way in \Cref{cha:distribution,cha:monitoring}.
Say that an \emph{operator} from type $A$ to $B$ is a function
$f$ from \texttt{List(Event)} to \texttt{List(Event)} that is \emph{monotone}:
If $l$ is a prefix of $l'$, then $f(l)$ is a prefix of $f(l')$.

Given an input stream type $S$ and output stream type $S'$:
\begin{itemize}
  \item \textbf{Type safety:} $f$ is \emph{linearization type safe} if whenever $\lintype{l}{S}$, $\lintype{f(l)}{S'}$.
  \item \textbf{Determinism:} $f$ is \emph{deterministic up to equivalence} if whenever $\equivtype{l}{l'}{S}$, $\equivtype{f(l)}{f(l')}{S}$.
\end{itemize}
...
$\singleton{T}$, denoting a stream with one element of type $T$.
...
For example, when processing the output of an aggregation operator it is often useful to know that the result has only one element, and process it accordingly.

Singletons can also be used to define relations that are set types, rather than bag types. The following abbreviation would accomplish this:
\[
  \mathsf{Set}(T) := \keyby{T}{\singleton{\unittype{}}}
\]
where $\unittype{}$ denotes the unit type.
...
Also, we could have $\deptype{t}{t}{\singleton{T}}$ or not; it doesn't affect things because a singleton stream only ever has one item, never two that could be dependent or independent.
Singleton streams are also harder to implement at runtime in a type-safe manner, in particular if there is an input stream to the system ensuring that it is a singleton stream requries a runtime check.

\subsection{Incrementality}

One thing missing from our discussion of streams in this chapter is incrementality.
We briefly remarked that stream operators should be monotone,
but batches and linearizations represent static views of an entire stream, bundled up as one object.
The later chapters will consider incremental computation more explicitly,
but it would also be useful to directly define generators and processors for incremental streams.
...
The stream types defined in this chapter differ from synchronization schemas as originally defined in our earlier work~\citeMain{pods21}
...
\Cref{prop:stream-types-less-general} shows that in fact the abstraction loses some dependence relations and so is less general; but we have not found the pathalogical cases it excludes to be useful in practice, such as $C_4$ and $P_4$-like dependence relations.
