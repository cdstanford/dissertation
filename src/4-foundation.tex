...
\end{proposition}
\begin{proof}
Enumerate the linearly many base types in $S$: these are instances of $T$ for $\hier{T}{S}$ or instances of $K$ for $\keyby{K}{S}$. For each pair of base types, check whether they are disjoint.
\end{proof}

\begin{theorem}
\label{thm:stream-relaxation-decidable}
Assume that groundedness is decidable in quadratic time or less.
Then \textsc{Subtyping} is decidable in quadratic time.
\end{theorem}
\begin{proof}
...
Consequently, checking order-equivalence is decidable in quadratic time as it is sufficient to check subtyping in both directions.
