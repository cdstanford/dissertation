...
Finally, \Cref{thm:stream-relaxation-decidable} states that subtyping for stream types is decidable in quadratic time.
...
\Cref{prop:compositionality-laws,prop:compositionaly-laws-additional} state compositionality laws for these properties (under sequential and parallel composition).
...
We say that a stream type $S$ is \emph{grounded} if all base types $T$ occuring in $S$ are disjoint.
...
or $\eventtype{e, e'}{S}$.
Each of these three cases matches exactly one rule either for $\mathcal{D}$ or $\mathcal{I}$, and not both.
\end{proof}
...
A \emph{labeled poset (polset)} over $X$ is a
...
Two polsets are \emph{equivalent} if the underlying posets are isomorphic and the isomorphism preserves the labeling $\ell$.
...
And (ii) states that $\le$ is the smallest ordering satisfying these conditions.
...
In this section, we define relaxation, a form of semantic subtyping: $S_1$ is a relaxation of $S_2$ means that a stream of type $S_2$ can be interpreted as a stream of type $S_1$ if the partial order is relaxed.
...
\begin{enumerate}
\item[(i)] $\event{S_1}$ is a supertype of $\event{S_2}$
(i.e., for all $t: S_2$, $t: S_1$); and
\item[(ii)] for all tuples $\eventtype{x, y}{S_2}$,
...
Stream type relaxation (on input two grounded stream types $S_1$ and $S_2$, checking if $S_1 \lesssim S_2$) is decidable in quadratic time.
\end{theorem}
\begin{proof}
...
Consequently, checking stream type equivalence is decidable in quadratic time as it is sufficient to check relaxation in both directions.
